...
export default {
  props: {
    ...
    position: {
      ...
      required: true,
      validator(value) {
        return ['top', 'bottom', 'left', 'center', 'right'].includes(value);
      },
    },
  },
  data() {
  ...
  methods: {
    ...
    selectNextPart() {
      this.selectedPartIndex = this.selectedPartIndex < (this.parts.length - 1)
        ? this.selectedPartIndex + 1 : 0;
    },
    selectPreviousPart() {
      this.selectedPartIndex = this.selectedPartIndex <= 0
        ? this.parts.length - 1 : this.selectedPartIndex - 1;
    },
  },
};
</script>
